export type ButtonOptions = {
  id?: string;
  text: string;
  variant?: "default" | "outline" | "danger" | "svg";
...
const buttonVariants: Record<string, string> = {
  ...
  svg: "hover:shadow-neon-cyan hover:bg-neon-cyan transition-all duration-500 ease-in-out"
};
...
export function Button({
  ...
  return `<button${idAttr} type="${type}" class="${classes}">${text}${svg}</button>`;
}
